...
inline char convert(char hex){
    if(((hex) >= '0')&&((hex) <= '9')){hex-='0';}
 else if(((hex) >= 'a')&&((hex) <= 'f')){hex-='a'-0xA;}
  else if((hex >= 'A')&&(hex <= 'F')){hex-='A'-0xA;}
  
  return hex;
}

void colorConverter(char* hexString, int *r, int*g,int *b)
{
  unsigned char value;
    
  value = convert(*hexString);
  *r = ((value) << 4) & 0xF0;
  hexString++;
  value = convert(*hexString);
  *r |= value;
